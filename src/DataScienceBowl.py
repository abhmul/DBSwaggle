...
def plot_3d(image, threshold=-300):
    """
    This function plots any ct scan. Input type must be ndarray or the ct_scan.
    Use arg threshold to set isosurface value of surface to be generated.
    """
...
def plot_3d(image, threshold=-300, num=0):
    ...
    ax.set_ylim(0, p.shape[1])
    ax.set_zlim(0, p.shape[2])
    plt.savefig('img_' + str(num) + '_threshold_' + str(threshold))
    plt.show()
...
def rescale(image, resize_min, resize_max, min_bound=0, max_bound=2000):
    return (resize_max - resize_min) * (image - min_bound) / (max_bound - min_bound) + resize_min
